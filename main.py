...
app.config['UPLOAD_FOLDER'] = "/var/www/pyxia/posts"
app.config['PFPUPLOAD_FOLDER'] = "/var/www/pyxia/static/pfp"
...
def change_pfp_post():
    ...
    if uploaded_file.filename != '':
        ...
        if scan_result == "malware":
            print("malware")
            post_path = Path(f"static/pfp/{current_user.id}{file_type}")
            os.remove(post_path)
            return redirect('/pyxia/profile/change_pfp')
...
def signup():
    if request.method == "GET":
        ...
        print("checking ages input")
        if age not in ages:
            return redirect('signup')
        chkemail = check_email(email.lower())
        if chkemail == True or chkemail == "True":
            session['already_exists'] = True
            return redirect('/signup')
        print("checked email")
        chkusr = check_user(username.lower())
...
def friendrequestspage():
    ...
    for user in requests:
        user_pfp = get_pfp(user[0])
        if user_pfp[0] == False:
            user.append(False)
            user_requests.append(user)
        else:
            user.append(user_pfp[1])
            user_requests.append(user)
    get_user_agent()
    if session["device_type"].lower() == "mobile":
        return render_template("mobile/friend_requests.html", requests=user_requests, dark_theme=theme, side_bar=side_bar, profile_pfp=pfp)
    else:
        return render_template("friend_requests.html", requests=user_requests, dark_theme=theme, side_bar=side_bar, profile_pfp=pfp)
